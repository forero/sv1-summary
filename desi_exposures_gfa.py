...
if (campaign=='minisv2'):
    ...
    f.close()
    tiles = [t['TILEID'] for t in obs]
    n = [t['NIGHTS'] for t in obs]
    nights = []
    for i in n:
        for j in i:
            nights.append(str(j['NIGHTID']))
    nights = list(set(nights))
    nights.sort()
    print('n', nights)
    gfafn = '/global/cfs/cdirs/desi/users/ameisner/GFA/conditions/offline_all_guide_ccds_SV1-thru_20201218.fits'
else:
